...
export default function App() {
  ...
  useEffect(() => {
    return () => {
      if (resetAckTimerRef.current) {
        clearTimeout(resetAckTimerRef.current);
      }
    };
  }, []);
  ...
  const acknowledgeResetClick = () => {
    setResetClicked(true);
    if (resetAckTimerRef.current) {
      clearTimeout(resetAckTimerRef.current);
    }
    resetAckTimerRef.current = setTimeout(() => {
  ...
  return (
    <div className="layout">
      ...
      <section className="controls panel">
        ...
        <div className="button-row">
          <button
            className={`pill action ${isMutating ? "busy" : ""} ${resetClicked ? "ack" : ""}`}
            onClick={() => {
              acknowledgeResetClick();
              resetExperiment();
            }}
            aria-busy={isMutating}
            disabled={isMutating}
          >
            {isMutating ? "EXECUTING..." : (resetClicked ? "CLICK REGISTERED" : "Reset Experiment Window")}
          </button>
        </div>
